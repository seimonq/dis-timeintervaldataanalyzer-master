...
public class TimeSeriesEvaluator extends BaseBitmapPreProcessorObservable {
	...
	private final static Logger LOG = LoggerFactory.getLogger(TimeSeriesEvaluator.class);
	 ...
	 */
	protected CombinedSlices createCombination(final TimeLevelMember member, DimensionSelector dimSel,
			final long[] bounds) {

//********************************************************************************************
//modified method for Kern's master theses
		...
		Bitmap combinedBitmap = new RoaringBitmap();
		Bitmap paResult = null;
		//search preaggregate for member
		SelectPreAggregates spa;
		int paLevel;

		try {
			spa = new SelectPreAggregates(dimModel.getPaModelHandler(), dimModel);
			paLevel = spa.timeMemberAvailable(dimSel, member);
		...
		for (final TimeMemberRange range : member.getRanges()) {
			...
			final SliceWithDescriptors<?>[] slices = index
				...
				// if preaggregated ranges exist which are covered completly
				// by the selected range use the preaggregated bitmap
			if(paLevel > 0 && range.getStart() == start && range.getEnd() == end) {

				paResult = spa.useTimeMember(member, range, dimSel,paLevel);
					// use preaggregated bitmap 
				combinedBitmap = combinedBitmap.or(paResult);
			}
				//if range does not fit into bounds do not look for preaggregates because only complete
				//	ranges are preaggregated and combine bitmaps from index instead
			else paLevel = -1;
			...
			for (final SliceWithDescriptors<?> slice : slices) {
				...
				combinedFactSet.combine(factSet);
					//if no preaggregated ranges exist combine slices from time axis index
				if(paLevel < 0) 
...
//******************************************************************************************
	...
	protected TimeSeriesCollection evaluateMixed(final SelectQuery query,
		...
		for (final TimeLevelMember member : members) {
			...
			// iterate over the ranges and create combined bitmaps and facts
			final CombinedSlices combined = createCombination(member, dim, bounds);
