...
import net.kern.master.preaggregates.operations.UpdatePreAggregates;
...
//extension for Kern's master theses
	...
	//trigger to update all affected preaggregates by loaded record (Arbeispaket 2)
		
		IDataRecord rec = this.idx.getRecord(id);

		//check if preAggregateHandler exists
		if(getDimensionModel().getPaModelHandler() != null) {
			try {
				UpdatePreAggregates updatePA = new UpdatePreAggregates
						 (this, getDimensionModel().getPaModelHandler());
				updatePA.updateAfterInsertion(rec);
...
//extension for Kern's master theses
		...
		if(getDimensionModel().getPaModelHandler() != null) {
			try {
				UpdatePreAggregates updatePa = new UpdatePreAggregates
						 (this, getDimensionModel().getPaModelHandler());
				updatePa.updateAfterDeletion(afterBitmap);
			}catch(PreAggregateException e) {
				if(LOG.isErrorEnabled()) LOG.error(e.getMessage());
			}
		}
