...
import net.kern.master.preaggregates.operations.CreatePreAggregates;
...
public class DimensionModel {
	...
	private final static Logger LOG = LoggerFactory.getLogger(DimensionModel.class);
...
//extension for Kern's master theses
	...
	private PreAggregateModelHandler paModelHandler;
	private boolean usePreAggregates;
	...
	public boolean isUsePreAggregates() {
		return usePreAggregates;
	}

	public void setUsePreAggregates(boolean usePreAggregates) {
		this.usePreAggregates = usePreAggregates;
	}

	public PreAggregateModelHandler getPaModelHandler() {
		return paModelHandler;
	}

	public void setPaModelHandler(PreAggregateModelHandler paModelHandler) {
		this.paModelHandler = paModelHandler;
	}
	/**
	 * trigger for Arbeitspaket 1
	 * 
	 * @param preaggregates
	 */
	public void createPreAggregates(Collection<PreAggregateSelector> preaggregates) {
		CreatePreAggregates cpa;

			//determine for each dimension whether it is a timedimension or descriptordimension
		for(PreAggregateSelector pas : preaggregates) {
			
				//create preaggregates for TimeLevelMember
			if(getDimension(pas.getDimensionId()) instanceof TimeGraph) {
				try {
					cpa = new CreatePreAggregates(this,getPaModelHandler(),index);
					cpa.createTimeMember(pas);
				} catch (PreAggregateException e) {
					if(LOG.isErrorEnabled()) LOG.error("PreAggregateException(createTimeMember): "+e.getMessage());
				}					
			}
			
				//create preaggregates for DescriptorGraphMember
			else if(getDimension(pas.getDimensionId()) instanceof DescriptorGraph) {

				try {
					cpa = new CreatePreAggregates(this,getPaModelHandler(),index);
					cpa.createDescriptorMember(pas);
				} catch (PreAggregateException e) {
					if(LOG.isErrorEnabled()) LOG.error("PreAggregateException(createDescriptorMember): "+e.getMessage());						}
			}
				//can not happen
			else {
				if(LOG.isErrorEnabled()) LOG.error("Unknown instance of a Graph has been passed");
			}
		}
